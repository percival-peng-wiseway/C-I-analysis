...
def _scenario_module(
    ...
    calculations = [
        ...
        _calculation("scenario.reactive_balance", "Reactive import after support", "post_kvar = measured_kvar - inverter_reactive_support", "Support cannot overcompensate the measured reactive import.", ["measured kvar", "reactive support kvar"], "solar_battery/ci_scenario_analysis.py::_execute_scenario"),
        ...
        _calculation("scenario.optimizer_objective", "Tariff-aware dispatch objective", "min sum_t(delta_t * (grid_import[t] * import_rate[t] - PV_export[t] * export_credit[t] + discharge[t] * 0.05)) + sum_d(demand_peak[d] * demand_rate[d])", "The export credit is currently fixed to zero. The AUD 0.05/kWh discharge term is a deterministic throughput tie-break and is not a finance cashflow.", ["interval imports", "zero export credit", "battery discharge", "approved demand components"], "solar_battery/ci_peak_shaving_optimizer.py::_model_bill"),
        ...
        _calculation("scenario.demand_peak", "Demand-peak epigraph", "peak[d] >= minimum_chargeable[d] and peak[d] >= post_import_kW[t] or exact post_import_kVA[t] for every interval in demand window d", "kVA components start with linear bounds and receive tangent cuts until exact nonlinear replay is within the fixed materiality gate or the run fails closed.", ["demand window", "post-import kW/kVA", "minimum chargeable demand", "demand rate"], "solar_battery/ci_peak_shaving_optimizer.py::_build_model"),
    ...
    return _module(
        ...
        calculations=calculations,
        models=[
            _model("scenario.pre_tariff_dispatch", "Pre-tariff feasibility dispatch", "PV-first self-consumption plus sampled peak-day envelope", "Rank measured-coverage physical performance before any customer-dollar meaning.", ["51 sampled peak thresholds on the selected measured peak day.", "Grid charging is disabled in the pre-tariff energy review.", "Ranking prioritises import-energy reduction, peak reduction and top-event coverage."], "solar_battery/ci_design_feasibility.py"),
            _model("scenario.highs_dispatch", "Tariff-aware optimizer", "HiGHS LP first; rerun as MILP if simultaneous charging and discharging is detected; then exact nonlinear rolling replay", "Minimise approved interval import and demand charges, zero-valued exports and the fixed discharge shadow cost while satisfying interval energy, SOC, power, demand and P-Q constraints.", ["48-hour planning horizon with 24-hour commits; December look-ahead wraps the same representative year's January without billing wrapped rows twice.", "The representative year begins and ends at the saved 100% initial SOC basis, and each rolling boundary must preserve at least the annual planner's feasible SOC.", "Grid charge plus PV charge cannot exceed battery charge power; discharge cannot exceed battery discharge power; charge and discharge modes are made mutually exclusive in the MILP fallback.", "The shared AC port is bidirectional and excludes the separately modelled PV-to-battery path.", "A conservative 16-segment P-Q polygon and iterative kVA tangent cuts are verified by exact nonlinear replay.", "Primary tolerance is AUD 0.01, exact-replay materiality is AUD 5, maximum kVA refinement is 24 iterations, and scenario time limit is 120 seconds.", "If the optimized exact bill fails to improve the idle bill by AUD 0.005, the saved dispatch falls back to idle."], "solar_battery/ci_peak_shaving_optimizer.py::execute_ci_peak_shaving_rolling"),
...
def _finance_module(
    *,
    ...
    calculations = [
        ...
        _calculation("finance.first_year_value", "First-year tariff value", "first_year_value_ex_GST = baseline_annual_cost_ex_GST - scenario_annual_cost_ex_GST", "Uses two stored representative-year tariff totals; it is not a generic retail-bill estimate.", ["baseline annual tariff total", "post-dispatch annual tariff total"], "solar_battery/ci_scenario_analysis.py::_annual_tariff_value", example=_tariff_value_example(tariff_scenarios)),
    ...
    return _module(
        ...
        calculations=calculations,
        models=[
            _model("finance.tariff_replay", "Representative-year tariff model", "Evidence-bound interval replay", "Repeat the approved analysis year and price baseline and scenario quantities with the active tariff.", ["Export credit is currently zero in dispatch and tariff value.", "Source-bill adjustment is excluded from scenario annual replay.", "The same tariff rates are used for baseline and scenario; future tariff changes and category-specific escalation are not modelled.", "Demand and tariff claims remain internal until evidence gates pass."], "solar_battery/ci_scenario_analysis.py::_annual_tariff_value"),
