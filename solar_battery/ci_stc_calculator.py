...
def calculate_stc_estimate(inputs: CiStcCalculatorInput) -> dict:
    ...
    battery_quantity = Decimal(str(inputs.battery_stc_count))
...
def stc_calculator_state(session, *, project_id, actor):
    ...
    requires_recalculation = legacy_capacity_reset or (saved is not None and "battery_installation_period" in saved)
    ...
    if draft is not None:
        draft.pop("battery_installation_period", None)
    inputs = CiStcCalculatorInput.model_validate(draft) if draft is not None else None
    return {"contract_version": "ci_stc_calculator_state_v3", "project_id": str(project_id),
            ...
            "estimate": calculate_stc_estimate(inputs) if inputs is not None and not requires_recalculation else None}
