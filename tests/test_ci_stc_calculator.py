...
def inputs(**changes):
    return CiStcCalculatorInput.model_validate({
        ...
        "battery_usable_capacity_kwh": 100, "battery_certificate_price": 39, **changes,
    })
...
def test_manual_example_no_caps_tiering_or_certificate_floor():
    ...
    assert result["battery_rebate_aud"] == 26520
    assert result["total_rebate_aud"] == 65340.60
...
def test_every_installation_period(period, factor):
    result = calculate_stc_estimate(inputs(battery_installation_period=period, battery_usable_capacity_kwh=10))
...
def test_project_worksheet_persists_without_touching_calculation_snapshots(tmp_path):
    ...
    with create_test_client(url) as client:
        ...
        assert response.json()["estimate"]["total_rebate_aud"] == 65340.60
