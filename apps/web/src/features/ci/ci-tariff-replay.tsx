  ...
  return <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
    ...
    <div className="grid gap-4 bg-slate-50/40 p-4 md:grid-cols-2 2xl:grid-cols-3 sm:p-5">
      {financeResult.solutions.map((solution) => {
        ...
        return <button aria-label={`View details for solution ${solution.financial_review_rank}: ${configuration(scenario)}`} className="group min-w-0 rounded-2xl border border-slate-200 bg-white p-5 text-left transition hover:border-cyan-300 hover:shadow-panel focus-visible:ring-2 focus-visible:ring-ring" key={solution.scenario_id} onClick={() => onOpen(solution.scenario_id)} type="button">
