...
describe("Device profile v5 API", () => {
  ...
  it("saves the full v5 profile as an AUD ex-GST contract", async () => {
  ...
  it("rejects legacy and duplicate profile contracts", () => {
    expect(() => assertCiDeviceProfileState({ ...state(), suggested_profile: { ...profile, contract_version: "ci_device_profile_v4" } })).toThrow("unsafe contract");
  ...
  it("strictly requires a boolean reactive-compensation selection on every inverter profile", () => {
    const missing = structuredClone(profile);
    delete (missing.solution_profiles.inverter_profiles[0] as unknown as { reactive_support_enabled?: unknown }).reactive_support_enabled;
    expect(() => assertCiDeviceProfileState({ ...state(), suggested_profile: missing })).toThrow("unsafe contract");

    const malformed = structuredClone(profile);
    (malformed.solution_profiles.inverter_profiles[0] as unknown as { reactive_support_enabled: unknown }).reactive_support_enabled = "yes";
    expect(() => assertCiDeviceProfileState({ ...state(), suggested_profile: malformed })).toThrow("unsafe contract");

    const enabledWithoutCapacity = structuredClone(profile);
    enabledWithoutCapacity.solution_profiles.inverter_profiles[0].maximum_reactive_power_kvar = 0;
    expect(() => assertCiDeviceProfileState({ ...state(), suggested_profile: enabledWithoutCapacity })).toThrow("unsafe contract");
  });
