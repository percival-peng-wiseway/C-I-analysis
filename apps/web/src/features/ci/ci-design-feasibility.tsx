  ...
  return (
    <section aria-label="Dispatch scenario analysis" className="grid overflow-hidden rounded-xl border border-slate-200 bg-white xl:grid-cols-[320px_minmax(0,1fr)]">
      <aside className="border-b border-slate-200 bg-slate-50/70 xl:border-b-0 xl:border-r">
        ...
        <div aria-label="Generated dispatch solutions" className="max-h-80 space-y-2 overflow-y-auto p-3 xl:max-h-[calc(100dvh-270px)]" role="list">
          {scenarios.map((scenario) => {
            ...
            return (
              <button aria-label={`Open solution ${scenario.physical_review_rank}: ${configuration(scenario)}`} aria-pressed={active} className={`w-full rounded-xl border p-3 text-left transition ${active ? "border-cyan-300 bg-cyan-50 shadow-sm" : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50"}`} key={scenario.scenario_id} onClick={() => setScenarioId(scenario.scenario_id)} type="button">
                ...
                <p className="mt-1 text-xs tabular-nums text-slate-500">{inverterDescription(scenario.authored_inputs)}</p>
              </button>
            );
          })}
...
function EnergyComparisonChart({ energy }: { energy: CiFeasibilityEnergyTotals }) {
  ...
  return <div aria-label="Annual grid import comparison" role="figure" className="min-w-0 space-y-5">{rows.map(row => <div key={row.label}>
    <div className="mb-2 flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 text-sm"><span className="text-slate-600">{row.label}</span><strong className="tabular-nums text-slate-950">{formatNumber(row.value / 1000)} MWh</strong></div>
    <div className="h-6 overflow-hidden rounded-md bg-slate-100"><div className="h-full rounded-md" style={{backgroundColor: row.color, width: `${row.value / maximum * 100}%`}} /></div>
  </div>)}</div>;
}
...
function PvDispositionChart({ energy }: { energy: CiFeasibilityEnergyTotals }) {
  ...
  return <div aria-label="Solar generation disposition" role="figure" className="min-w-0">
    <p className="text-sm leading-6 text-slate-600">{formatNumber(energy.pv_generation_kwh / 1000)} MWh generated · {formatNumber(energy.pv_self_consumption_percent)}% self-consumed</p>
    <div className="my-4 flex h-8 overflow-hidden rounded-md bg-slate-100" aria-hidden="true">{categories.map(item => <span key={item.label} style={{backgroundColor: item.color, width: `${item.value / total * 100}%`}} />)}</div>
    <dl className="grid gap-x-6 gap-y-3 sm:grid-cols-2">{categories.map(item => <div className="flex min-w-0 flex-wrap items-baseline justify-between gap-2 text-sm" key={item.label}><dt className="flex items-center gap-2 text-slate-600"><span className="size-2 shrink-0 rounded-sm" style={{backgroundColor: item.color}} />{item.label}</dt><dd className="font-semibold tabular-nums">{formatNumber(item.value / 1000)} MWh</dd></div>)}</dl>
  </div>;
}

function PeakDayChart({ scenario }: { scenario: CiFeasibilityScenario }) {
  const { ref, width } = useChartWidth(820);
  const points = scenario.peak_day.points; const height = 330; const left = 54; const top = 22; const bottom = 42; const right = 18;
...
function TopPeakEventsChart({ performance }: { performance: CiFeasibilityPerformance }) {
  ...
  const height = 320; const left = 54; const right = 16; const top = 24; const bottom = 66; const plotHeight = height - top - bottom; const groupWidth = (width - left - right) / events.length; const maximum = Math.max(1, ...events.map((event) => event.baseline_kw)) * 1.08; const y = (value: number) => top + plotHeight * (1 - value / maximum);
  return <div ref={ref} className="min-w-0"><svg aria-label="Top measured peak event comparison" className="block h-auto w-full" role="img" viewBox={`0 0 ${width} ${height}`}>{[0, .25, .5, .75, 1].map((tick) => <g key={tick}><line stroke="#dbe3ec" x1={left} x2={width-right} y1={y(maximum*tick)} y2={y(maximum*tick)} /><text fill="#64748b" fontSize="11" textAnchor="end" x={left-8} y={y(maximum*tick)+4}>{formatNumber(maximum*tick,0)}</text></g>)}{events.map((event, index) => { const x = left + index * groupWidth + groupWidth * .14; const barWidth = Math.max(1, groupWidth * .18); return <g key={event.timestamp}><title>{shortDate(event.timestamp)}: measured {formatNumber(event.baseline_kw)} kW, PV only {formatNumber(event.pv_only_import_kw)} kW, PV + battery {formatNumber(event.grid_import_kw)} kW</title><rect fill="#334155" height={top+plotHeight-y(event.baseline_kw)} width={barWidth} x={x} y={y(event.baseline_kw)} /><rect fill="#eab308" height={top+plotHeight-y(event.pv_only_import_kw)} width={barWidth} x={x+barWidth+2} y={y(event.pv_only_import_kw)} /><rect fill="#0891b2" height={top+plotHeight-y(event.grid_import_kw)} width={barWidth} x={x+(barWidth+2)*2} y={y(event.grid_import_kw)} /><text fill="#64748b" fontSize="10" textAnchor="middle" x={x+barWidth+2} y={height-38}>#{event.rank}</text>{width >= 560 ? <text fill="#64748b" fontSize="9" textAnchor="middle" transform={`rotate(-28 ${x+barWidth+2} ${height-16})`} x={x+barWidth+2} y={height-16}>{shortDate(event.timestamp)}</text> : null}</g>; })}<text fill="#475569" fontSize="11" x="8" y="17">kW</text></svg><Legend items={[["#334155", "Measured"], ["#eab308", "PV only"], ["#0891b2", "PV + battery"]]} /></div>;
}
...
function BatteryOperationChart({ scenario }: { scenario: CiFeasibilityScenario }) {
  ...
  const height = 430; const left = 78; const right = 28; const plotWidth = width - left - right;
  ...
  return <div>
    <div ref={ref} className="min-w-0">
      <svg aria-label="Battery state of charge and power chart" className="block h-auto w-full" role="img" viewBox={`0 0 ${width} ${height}`}>
        ...
        {(width < 480 ? [0, .5, 1] : [0, .25, .5, .75, 1]).map((tick) => { const index = Math.min(points.length - 1, Math.round((points.length - 1) * tick)); return <text fill="#64748b" fontSize="11" key={tick} textAnchor={tick === 0 ? "start" : tick === 1 ? "end" : "middle"} x={x(index)} y={height-14}>{points[index].time_label}</text>; })}
      </svg>
    </div>
...
function DailyProfileCloud({ result }: { result: CiDesignFeasibilityResult }) {
  const { ref, width } = useChartWidth(980);
  const cloud = result.baseline.daily_profile_cloud; const height = 330; const left = 52; const top = 20; const bottom = 40; const right = 14;
  ...
  return <div ref={ref} className="min-w-0"><svg aria-label="Daily measured demand profile cloud" className="block h-auto w-full" role="img" viewBox={`0 0 ${width} ${height}`}><rect fill="#fbfcfe" x={left} y={top} width={width-left-right} height={height-top-bottom} />{cloud.sampled_daily_profiles.map((profile) => <path d={path(profile.values_kw)} fill="none" key={profile.date} opacity=".16" stroke="#94a3b8" strokeWidth=".75" />)}<path d={path(cloud.average_day_kw)} fill="none" stroke="#7e22ce" strokeWidth="3" /><path d={path(cloud.selected_peak_day_kw)} fill="none" stroke="#d946ef" strokeWidth="3" />{(width < 480 ? [0, .5, 1] : [0, .25, .5, .75, 1]).map((tick) => { const index = Math.min(count - 1, Math.round((count - 1) * tick)); return <text fill="#64748b" fontSize="11" textAnchor={tick === 0 ? "start" : tick === 1 ? "end" : "middle"} x={x(index)} y={height - 13} key={tick}>{cloud.time_labels[index]}</text>; })}<text fill="#475569" fontSize="11" x="8" y="16">kW</text></svg><Legend items={[["#94a3b8", "Sampled days"], ["#7e22ce", "Average day"], ["#d946ef", "Selected peak day"]]} /></div>;
}
...
function configuration(scenario: CiFeasibilityScenario) { return `${formatCapacity(scenario.authored_inputs.pv_capacity_kwp_dc)} kWp PV · ${formatCapacity(scenario.authored_inputs.nominal_capacity_kwh)} kWh battery · ${inverterDescription(scenario.authored_inputs)}`; }
