          ...
          {scenarios.map((scenario) => {
            ...
            return (
              <button aria-label={`Open solution ${scenario.physical_review_rank}: ${configuration(scenario)}`} aria-pressed={active} className={`w-full rounded-xl border p-3 text-left transition ${active ? "border-cyan-300 bg-cyan-50 shadow-sm" : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50"}`} key={scenario.scenario_id} onClick={() => setScenarioId(scenario.scenario_id)} type="button">
                ...
                <p className="mt-2 text-sm font-semibold tabular-nums text-slate-950">{formatNumber(scenario.authored_inputs.pv_capacity_kwp_dc)} kWp PV · {formatNumber(scenario.authored_inputs.nominal_capacity_kwh)} kWh</p>
                <p className="mt-1 text-xs tabular-nums text-slate-500">{formatNumber(scenario.authored_inputs.pv_inverter_capacity_kw_ac)} kW hybrid inverter / PCS</p>
              </button>
            );
          })}
...
function configuration(scenario: CiFeasibilityScenario) { return `${formatNumber(scenario.authored_inputs.pv_capacity_kwp_dc)} kWp PV · ${formatNumber(scenario.authored_inputs.nominal_capacity_kwh)} kWh battery · ${formatNumber(scenario.authored_inputs.pv_inverter_capacity_kw_ac)} kW hybrid inverter / PCS`; }
