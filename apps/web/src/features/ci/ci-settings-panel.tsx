...
export function CiSettingsPanel({ onClose }: { onClose: () => void }) {
  ...
  const save = useMutation({
    ...
    onSuccess: (state) => {
      ...
      void queryClient.invalidateQueries({ queryKey: ["ci-project-feasibility"] });
      void queryClient.invalidateQueries({ queryKey: ["ci-project-tariff-replay"] });
...
function SolarProfileEditor({ isDefault, onDefault, onUpdate, profile }: { isDefault: boolean; onDefault: () => void; onUpdate: (profile: CiSolarSolutionProfile) => void; profile: CiSolarSolutionProfile }) {
  return (
    <ProfileEditorShell isDefault={isDefault} kind="solar" onDefault={onDefault} onStatusChange={(status) => onUpdate({ ...profile, status })} onVersionChange={(version) => onUpdate({ ...profile, version })} profile={profile}>
      ...
      <NumberProfileField referenceOnly label="Rated power" onChange={(rated_power_w) => onUpdate({ ...profile, rated_power_w })} suffix="W" value={profile.rated_power_w} />
      <NumberProfileField referenceOnly label="Module efficiency" onChange={(module_efficiency_percent) => onUpdate({ ...profile, module_efficiency_percent })} suffix="%" value={profile.module_efficiency_percent} />
      <NumberProfileField referenceOnly label="Temperature coefficient" onChange={(temperature_coefficient_percent_per_c) => onUpdate({ ...profile, temperature_coefficient_percent_per_c })} suffix="% / °C" value={profile.temperature_coefficient_percent_per_c} />
      <NumberProfileField referenceOnly label="Annual degradation" onChange={(annual_degradation_percent) => onUpdate({ ...profile, annual_degradation_percent })} suffix="% / yr" value={profile.annual_degradation_percent} />
...
function BatteryProfileEditor({ isDefault, onDefault, onUpdate, profile }: { isDefault: boolean; onDefault: () => void; onUpdate: (profile: CiBatterySolutionProfile) => void; profile: CiBatterySolutionProfile }) {
  ...
  return (
    <ProfileEditorShell isDefault={isDefault} kind="battery" onDefault={onDefault} onStatusChange={(status) => onUpdate({ ...profile, status })} onVersionChange={(version) => onUpdate({ ...profile, version })} profile={profile}>
      ...
      <NullableNumberProfileField referenceOnly label="Standby loss" onChange={(standby_loss_percent_per_month) => onUpdate({ ...profile, standby_loss_percent_per_month })} suffix="% / month" value={profile.standby_loss_percent_per_month} />
      <NullableNumberProfileField referenceOnly label="Annual capacity degradation" onChange={(annual_capacity_degradation_percent) => onUpdate({ ...profile, annual_capacity_degradation_percent })} suffix="% / yr" value={profile.annual_capacity_degradation_percent} />
      <NullableNumberProfileField referenceOnly integer label="Minimum units" onChange={(minimum_units) => onUpdate({ ...profile, minimum_units })} value={profile.minimum_units} />
      <NullableNumberProfileField referenceOnly integer label="Maximum units" onChange={(maximum_units) => onUpdate({ ...profile, maximum_units })} value={profile.maximum_units} />
      <SourceFields onUpdate={(source) => onUpdate({ ...profile, ...source })} profile={profile} />
    </ProfileEditorShell>
  );
}
...
function InverterProfileEditor({ onUpdate, profile }: { onUpdate: (profile: CiInverterSolutionProfile) => void; profile: CiInverterSolutionProfile }) {
  return (
    <ProfileEditorShell isDefault={false} kind="inverter" onStatusChange={(status) => onUpdate({ ...profile, status })} onVersionChange={(version) => onUpdate({ ...profile, version })} profile={profile}>
      ...
      <BooleanProfileField label="Reactive compensation" onChange={(reactive_support_enabled) => onUpdate({ ...profile, reactive_support_enabled })} value={profile.reactive_support_enabled} />
      <NumberProfileField label="Reactive support cap per inverter" onChange={(maximum_reactive_power_kvar) => onUpdate({ ...profile, maximum_reactive_power_kvar })} suffix="kvar" value={profile.maximum_reactive_power_kvar} />
      <NumberProfileField referenceOnly label="Leading power factor limit" onChange={(power_factor_leading_limit) => onUpdate({ ...profile, power_factor_leading_limit })} value={profile.power_factor_leading_limit} />
      <NumberProfileField referenceOnly label="Lagging power factor limit" onChange={(power_factor_lagging_limit) => onUpdate({ ...profile, power_factor_lagging_limit })} value={profile.power_factor_lagging_limit} />
      <BooleanProfileField referenceOnly label="P–Q capability curve available" onChange={(pq_capability_curve_available) => onUpdate({ ...profile, pq_capability_curve_available })} value={profile.pq_capability_curve_available} />
      <BooleanProfileField referenceOnly label="Reactive power at zero active power" onChange={(reactive_power_at_zero_active_power) => onUpdate({ ...profile, reactive_power_at_zero_active_power })} value={profile.reactive_power_at_zero_active_power} />
      <BooleanProfileField referenceOnly label="Night reactive capability" onChange={(night_reactive_capability) => onUpdate({ ...profile, night_reactive_capability })} value={profile.night_reactive_capability} />
      <NumberProfileField referenceOnly label="European efficiency" onChange={(european_efficiency_percent) => onUpdate({ ...profile, european_efficiency_percent })} suffix="%" value={profile.european_efficiency_percent} />
      <NumberProfileField referenceOnly label="Maximum efficiency" onChange={(maximum_efficiency_percent) => onUpdate({ ...profile, maximum_efficiency_percent })} suffix="%" value={profile.maximum_efficiency_percent} />
      <SourceFields onUpdate={(source) => onUpdate({ ...profile, ...source })} profile={profile} />
    </ProfileEditorShell>
  );
}
...
function ProfileEditorShell({ children, isDefault, kind, onDefault, onStatusChange, onVersionChange, profile }: { children: ReactNode; isDefault: boolean; kind: SolutionProfileKind; onDefault?: () => void; onStatusChange: (status: CiSolutionProfileStatus) => void; onVersionChange: (version: number) => void; profile: CiSolarSolutionProfile | CiBatterySolutionProfile | CiInverterSolutionProfile }) {
  return (
    <section aria-label={`Edit ${kind} profile`} className="rounded-xl border border-slate-200 bg-slate-50/60 p-4">
      ...
      <p className="mb-3 text-xs leading-5 text-slate-600">Fields marked Reference only are saved with the profile and do not change the current calculations. Regenerate solutions after changing active performance inputs. Device ageing is not simulated; Finance uses its separate Value degradation assumption.</p>
      <div className="grid gap-3 sm:grid-cols-2">
...
function NumberProfileField({ referenceOnly = false, integer = false, label, onChange, suffix, value }: { referenceOnly?: boolean; integer?: boolean; label: string; onChange: (value: number) => void; suffix?: string; value: number }) {
  return <label className="grid gap-1 text-xs font-medium text-slate-600"><span>{label}{referenceOnly ? <span className="ml-1 text-[10px] font-normal text-amber-800">Reference only</span> : null}</span><span className="relative"><input aria-label={label} className={`w-full rounded-md border border-slate-200 bg-white px-2.5 py-2 text-sm tabular-nums ${suffix ? "pr-20" : ""}`} onChange={(event) => onChange(integer ? Math.round(Number(event.target.value)) : Number(event.target.value))} step={integer ? "1" : "any"} type="number" value={value} />{suffix ? <span className="absolute right-2.5 top-2 text-xs text-slate-400">{suffix}</span> : null}</span></label>;
}

function NullableNumberProfileField({ referenceOnly = false, integer = false, label, onChange, suffix, value }: { referenceOnly?: boolean; integer?: boolean; label: string; onChange: (value: number | null) => void; suffix?: string; value: number | null }) {
  return <label className="grid gap-1 text-xs font-medium text-slate-600"><span>{label}{referenceOnly ? <span className="ml-1 text-[10px] font-normal text-amber-800">Reference only</span> : null}</span><span className="relative"><input aria-label={label} className={`w-full rounded-md border border-slate-200 bg-white px-2.5 py-2 text-sm tabular-nums ${suffix ? "pr-20" : ""}`} onChange={(event) => { const raw = event.target.value; onChange(raw === "" ? null : integer ? Math.round(Number(raw)) : Number(raw)); }} placeholder="Required to publish" step={integer ? "1" : "any"} type="number" value={value ?? ""} />{suffix ? <span className="absolute right-2.5 top-2 text-xs text-slate-400">{suffix}</span> : null}</span></label>;
}

function BooleanProfileField({ referenceOnly = false, label, onChange, value }: { referenceOnly?: boolean; label: string; onChange: (value: boolean) => void; value: boolean }) {
  return <label className="grid gap-1 text-xs font-medium text-slate-600"><span>{label}{referenceOnly ? <span className="ml-1 text-[10px] font-normal text-amber-800">Reference only</span> : null}</span><select aria-label={label} className="rounded-md border border-slate-200 bg-white px-2.5 py-2 text-sm" onChange={(event) => onChange(event.target.value === "yes")} value={value ? "yes" : "no"}><option value="yes">Yes</option><option value="no">No</option></select></label>;
}
...
function PriceGridHeader({ first }: { first: string }) {
  return <div className="grid grid-cols-[90px_repeat(3,minmax(110px,1fr))] gap-2 px-2 pb-2 text-[10px] font-semibold uppercase tracking-wide text-slate-400"><span>{first}</span><span className="text-right">Capital</span><span className="text-right">Replacement (reference)</span><span className="text-right">O&amp;M / yr (reference)</span></div>;
}
...
function validateDraft(profile: CiDeviceProfile) {
  ...
  const financeValid =
    validNonNegativeMoney(profile.installation_misc_cost_aud) &&
