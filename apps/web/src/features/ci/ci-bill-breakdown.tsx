        ...
        <div className="grid items-stretch gap-4 xl:grid-cols-[1.35fr_0.65fr]">
          <section aria-labelledby="charge-breakdown-title" className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            ...
            {categoryDetailAvailable ? (
              <ChargeCategoryTable categories={categories} total={categoryTotal} />
            ) : (
...
function ChargeCategoryTable({ categories, total }: { categories: Array<{ amount: number; colour: string; key: string; label: string }>; total: number }) {
  return (
    <div className="overflow-x-auto">
      <table aria-label="Detected invoice charge categories" className="w-full min-w-[520px] border-collapse text-sm">
        <thead className="bg-slate-50 text-left text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">
          <tr><th className="px-5 py-3">Charge category</th><th className="px-5 py-3 text-right">Share</th><th className="px-5 py-3 text-right">Amount ex GST</th></tr>
        </thead>
          ...
          {categories.map((item) => {
            ...
            return (
              <tr className="hover:bg-slate-50/70" key={item.key}>
                ...
                <td className="px-5 py-3 text-right tabular-nums text-slate-500">{formatPercent(share)}</td>
                <td className="px-5 py-3 text-right font-semibold tabular-nums text-slate-950">{formatAud(item.amount)}</td>
              </tr>
            );
          })}
        </tbody>
        <tfoot className="border-t border-slate-300 bg-slate-100/80">
          <tr><th className="px-5 py-3 text-left font-semibold text-slate-950" colSpan={2}>Detected charge total</th><td className="px-5 py-3 text-right font-semibold tabular-nums text-slate-950">{formatAud(total)}</td></tr>
        </tfoot>
      </table>
      <p className="border-t border-slate-200 px-5 py-3 text-xs text-slate-500">The source invoice provides category totals. Unit rates, quantities and loss factors are not inferred here.</p>
    </div>
  );
}
...
function PowerFactorGauge({ value }: { value: number | null }) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 sm:col-span-2 xl:col-span-1">
      ...
      <p className="mt-1 text-[10px] text-slate-500">At maximum recorded demand</p>
      <div aria-label={value === null ? "Power factor unavailable" : `Power factor at maximum demand ${value.toFixed(3)}`} className="mt-4 h-2.5 overflow-hidden rounded-full bg-slate-100" role="img">
