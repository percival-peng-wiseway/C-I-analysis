...
const emptyDraft: Draft = { solar_installation_year: "2026", solar_zone: "4", pv_capacity_kwp: "",
  ...
  battery_usable_capacity_kwh: "", battery_certificate_price: "39" };
...
function StcWorksheet({ projectId }: { projectId: string }) {
  ...
  useEffect(() => { if (state.data && draft === null) setDraft(state.data.estimate ? draftFrom(state.data.estimate.inputs) : { ...emptyDraft }); }, [state.data, draft]);
  ...
  const valid = !!draft && Object.entries(draft).every(([key, value]) => key === "battery_installation_period" || (value.trim() !== "" && Number.isFinite(Number(value)) && Number(value) >= 0))
    ...
    && Number(draft.pv_capacity_kwp) <= 1_000_000 && Number(draft.battery_usable_capacity_kwh) <= 1_000_000;
      ...
      {state.isError ? <p role="alert">STC worksheet could not be loaded. <button type="button" className="underline" onClick={() => { void state.refetch(); }}>Retry</button></p> : !draft ? <p>Loading STC worksheet…</p> : <>
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <section aria-label="Battery STC calculator" className="space-y-3 rounded-lg border border-slate-200 bg-slate-50/40 p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">{field("battery_usable_capacity_kwh", "Battery usable capacity (kWh)")}{field("battery_certificate_price", "Battery STC price (AUD / certificate)")}</div>
            <p className="text-xs text-slate-500">Usable kWh × installation-period factor × price</p>
            {saved && !dirty ? <><p className="text-xs">{saved.inputs.battery_usable_capacity_kwh} × {saved.battery_factor} × {saved.inputs.battery_certificate_price}</p><p className="text-lg font-semibold">Battery rebate: {money(saved.battery_rebate_aud)}</p></> : null}
          </section>
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-emerald-50 p-4">
          <p className="font-semibold" aria-live="polite">{saved && !dirty ? `Saved total rebate estimate: ${money(saved.total_rebate_aud)}` : "Enter both capacities (use 0 if not required), then calculate and save."}</p>
