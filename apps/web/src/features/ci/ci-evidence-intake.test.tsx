...
it("inspects the usual bill and NEM12 pair and continues to system design", async () => {
  ...
  const firstView = render(
    <QueryClientProvider client={new QueryClient()}>
      <CiEvidenceIntake onReady={onUseNem12} profileReady projectId="project-1" />
    </QueryClientProvider>,
  );
  ...
  await user.click(screen.getByRole("button", { name: "Inspect both files" }));

  expect(await screen.findByText("The bill and NEM12 form a coherent input pair")).toBeTruthy();
  expect(screen.getAllByText("B1 · E1 · K1 · Q1")).toHaveLength(2);
  ...
  await user.click(screen.getByRole("button", { name: "Continue to System design" }));
  expect(onUseNem12).toHaveBeenCalledWith();

  firstView.unmount();
  render(
    <QueryClientProvider client={new QueryClient()}>
      <CiEvidenceIntake onReady={vi.fn()} profileReady projectId="project-1" />
    </QueryClientProvider>,
  );
  expect(await screen.findByRole("heading", { name: "Saved project evidence" })).toBeTruthy();
  expect(screen.getByText("bill.pdf")).toBeTruthy();
  expect(screen.getByText("meter.csv")).toBeTruthy();
  expect(screen.queryByLabelText("Electricity bill PDF")).toBeNull();
  expect(screen.getByRole("heading", { name: "15-minute demand heatmap" })).toBeTruthy();
});
...
it("confirms a retailer-neutral bill without adding a company adapter", async () => {
  ...
  const baseResult = {
    ...
    bill: {
      ...
      network_tariff_code: "LLVT2", consumption_kwh: 288, highest_metered_demand_kva: 15,
  ...
  render(<QueryClientProvider client={new QueryClient()}><CiEvidenceIntake onReady={vi.fn()} profileReady projectId="project-1" /></QueryClientProvider>);
