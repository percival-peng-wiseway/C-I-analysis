...
export function CiEvidenceIntake({
  ...
  const result = !replacing ? saved?.inspection ?? inspection.data : undefined;
  const evidenceBusy = inspection.isPending || savedReview.isPending;
  const fullTariffReady = result?.nem12.full_tariff_analysis_ready ?? false;
  const needsNmiCorrection = result?.pair_checks.some((check) => check.code === "site_identity_match" && !check.passed) ?? false;
  ...
  return (
    <section className="scroll-mt-20 space-y-4" id="evidence-intake">
      ...
      <div className="flex flex-wrap justify-end gap-2">
          ...
          {saved && replacing ? <Button disabled={evidenceBusy} onClick={() => { setReplacing(false); setBill(null); setNem12(null); resetResult(); }} type="button" variant="ghost">Cancel replacement</Button> : null}
      </div>
      ...
      {result ? (
        <div className="space-y-4">
          ...
          {result.bill.extraction_audit ? <BillExtractionAudit audit={result.bill.extraction_audit} /> : null}
          {result.bill.review_status === "confirmation_required" || needsNmiCorrection ? (
            <BillReviewForm
              key={result.bill.fingerprint}
              bill={result.bill}
              busy={savedReview.isPending}
              requireNmiCorrection={needsNmiCorrection}
...
const billCategories = [
  ["energy_charges", "Energy charges"], ["network_charges", "Network charges"],
  ["regulated_charges", "Regulated charges"], ["environmental_charges", "Environmental charges"],
  ["metering_charges", "Metering charges"], ["additional_charges", "Additional charges / credits"],
] as const;

function BillReviewForm({ bill, busy, initialNetworkTariffCode, requireNmiCorrection, onConfirm }: { bill: CiEvidenceIntakeResult["bill"]; busy: boolean; initialNetworkTariffCode: string; requireNmiCorrection: boolean; onConfirm: (review: CiBillReviewInput) => void }) {
  ...
  const [lineItemsReviewed, setLineItemsReviewed] = useState(false);
  const hasFailedLines = bill.extraction_audit?.line_items.some((line) => !line.passed) ?? false;
  const [categories, setCategories] = useState<Record<string, string>>(() => Object.fromEntries(billCategories.map(([key]) => [key, numberInput(bill.charge_categories_ex_gst_aud[key] ?? null)])));
  const hasCategories = Object.values(categories).some((value) => value !== "");
  const categoriesValid = !hasCategories || billCategories.every(([key]) => categories[key] !== "" && Number.isFinite(Number(categories[key])) && (key === "additional_charges" || Number(categories[key]) >= 0));
  useEffect(() => {
  ...
  const requiresNmi = requireNmiCorrection || bill.site_identity_status === "missing";
  ...
  const valid = (!hasFailedLines || lineItemsReviewed) && categoriesValid && fields.retailer.trim() && fields.invoice_kind.trim() && fields.billing_period_start && fields.billing_period_end && fields.network_tariff_code.trim() && (!requiresNmi || /^[A-Za-z0-9]{10,11}$/.test(fields.nmi)) && numericKeys.every((key) => fields[key] !== "" && Number.isFinite(Number(fields[key])) && Number(fields[key]) >= 0) && Number(fields.power_factor_at_highest_demand) <= 1;
  return (
    <Card className="border-amber-300 bg-amber-50/40">
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><CardTitle as="h3">Confirm bill fields</CardTitle><CardDescription>Check the extracted fields and any reported differences against the PDF. Confirmed corrections are saved and reconciled again.</CardDescription></div>
      ...
      <CardContent>
        <form className="space-y-4" onSubmit={(event) => {
          ...
          onConfirm({
            confirmed: true,
            expected_bill_fingerprint: bill.fingerprint,
            ...(hasFailedLines ? { line_items_reviewed: lineItemsReviewed } : {}),
            ...(hasCategories ? { charge_categories_ex_gst_aud: Object.fromEntries(billCategories.map(([key]) => [key, Number(categories[key])])) } : {}),
          ...
          {bill.extraction_method === "manual_review_only" ? <p className="rounded-lg border border-amber-300 bg-white p-3 text-sm text-amber-950">No reliable text was extracted from this PDF. Enter the fields from the visible bill and check any extraction issues above.</p> : null}
          ...
          <fieldset className="space-y-3 rounded-lg border border-amber-200 p-3">
            <legend className="px-1 text-sm font-semibold">Charge categories · AUD ex GST</legend>
            <p className="text-xs text-muted-foreground">Enter all six categories to reconcile the breakdown, or leave all blank for a totals-only review. Enter zero only when verified on the bill; credits use a negative amount. A totals-only review does not approve tariff calculations.</p>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{billCategories.map(([key, label]) => <ReviewField key={key} label={label} min={key === "additional_charges" ? undefined : "0"} onChange={(value) => setCategories((current) => ({ ...current, [key]: value }))} step="0.01" type="number" value={categories[key]} />)}</div>
          </fieldset>
          {hasFailedLines ? <label className="flex items-start gap-2 text-sm"><input checked={lineItemsReviewed} className="mt-1" onChange={(event) => setLineItemsReviewed(event.target.checked)} type="checkbox" /><span>I checked the flagged charge rows against the PDF and corrected the invoice fields above. Tariff rates will require separate profile review.</span></label> : null}
...
function EvidenceFileCard({ accept, description, disabled, file, icon, inputLabel, label, onChange, savedName }: { accept: string; description: string; disabled: boolean; file: File | null; icon: LucideIcon; inputLabel: string; label: string; onChange: (file: File | null) => void; savedName: string | null }) {
  ...
  return (
    <article className="flex min-h-56 flex-col rounded-xl border border-slate-200 bg-white p-4">
      ...
      <label className={`mt-auto min-w-0 pt-4 ${disabled ? "cursor-wait opacity-60" : "cursor-pointer"}`}>
        ...
        <input accept={accept} aria-label={inputLabel} className="sr-only" disabled={disabled} onChange={(event) => onChange(event.target.files?.item(0) ?? null)} type="file" />
