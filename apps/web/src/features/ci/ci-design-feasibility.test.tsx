...
const scenarios = Array.from({ length: 12 }, (_, index) => scenario({ id: `solution-${index + 1}`, label: `Solution ${index + 1}`, rank: index + 1, pv: 100 + index * 5, inverter: 80 + index * 4, battery: 200 + index * 10, reduction: 30 + index }));
const result = {
  contract_version: "ci_design_feasibility_v4", status: "ready", analysis_mode: "pre_tariff_physical_feasibility", customer_facing_permission: false, recommendation_permitted: false, tariff_evaluated: false, currency_values_permitted: false,
...
describe("C&I design feasibility visuals", () => {
  it("shows every generated solution on the left and selectable dispatch metrics on the right", async () => {
    ...
    expect(screen.getByRole("heading", { name: "100 kWp PV · 200 kWh battery · 80 kW hybrid inverter / PCS" })).toBeTruthy();
