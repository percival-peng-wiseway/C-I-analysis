...
function GeneratedSolutionQuotes({ addCustomError, analysisError, analysisPending, generationSummary, isAddingCustom, isLoading, onAddCustom, onAnalyze, onQuoteChange, onRetry, onSelectionChange, onSelectAll, preview, previewError, quotes, selectedSolutions, siteAcHeadroomKw }: {
  ...
  if (isLoading && !preview) return <section aria-busy="true" aria-labelledby="solutions-loading-title" className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6"><h3 className="text-xl font-semibold text-slate-950" id="solutions-loading-title">Preparing solutions</h3><p className="mt-4 flex items-center gap-2 text-sm text-slate-600" role="status"><RefreshCw aria-hidden="true" className="size-4 animate-spin motion-reduce:animate-none" />Calculating Net CAPEX for every feasible solution…</p><div aria-hidden="true" className="mt-5 space-y-3">{[0, 1, 2].map((row) => <div className="h-12 rounded-lg bg-slate-50" key={row} />)}</div></section>;
  if (previewError || !preview) return <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 sm:p-6"><h2 className="font-semibold text-amber-950">Net CAPEX is not ready</h2><p className="mt-1 text-sm leading-6 text-amber-900" role="alert">{previewError ?? "Save the equipment and rebate assumptions, then try again."}</p><Button className="mt-4" onClick={onRetry} type="button" variant="outline"><RefreshCw aria-hidden="true" className="size-4" />Retry pricing</Button></section>;
  ...
  const analysisBlocker = selectedCount === 0
    ...
    : !validQuotes
      ...
      : null;
  const controlsBusy = isLoading || analysisPending || isAddingCustom;
  return (
    <section aria-busy={isLoading || isAddingCustom} aria-labelledby="generated-solution-quotes-title" className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      <header className="flex flex-wrap items-start justify-between gap-4 p-5 sm:p-6">
        <div>
          <div className="flex items-center gap-3"><h3 className="text-xl font-semibold tracking-tight text-slate-950" id="generated-solution-quotes-title">Solutions</h3><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold tabular-nums text-slate-600">{preview.candidate_count}</span></div>
          {generationSummary ? (
            <div aria-label="Solution generation summary" className="mt-2 flex flex-wrap gap-2 text-xs text-slate-600">
              <span>{generationSummary.requested_count} configured {generationSummary.requested_count === 1 ? "combination" : "combinations"}</span>
        ...
        <div className="flex flex-col items-end gap-2">
          <Button aria-controls="custom-solution-form" aria-describedby={customSolutionLimitReached ? "custom-solution-limit" : undefined} aria-expanded={showCustom} disabled={customSolutionLimitReached || controlsBusy} onClick={() => { setCustomValidationError(null); setShowCustom((current) => !current); }} type="button" variant="outline"><Plus aria-hidden="true" className="size-4" />Add custom solution</Button>
      ...
      {showCustom ? (
        <form aria-labelledby="custom-solution-title" className="mx-5 mb-5 rounded-xl border border-cyan-200 bg-cyan-50/40 p-4 sm:mx-6 sm:p-5" id="custom-solution-form" noValidate onSubmit={(event) => { event.preventDefault(); if (!controlsBusy) void submitCustom(); }}>
          <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-semibold text-slate-950" id="custom-solution-title">Custom solution &amp; quotation</h3><Button disabled={controlsBusy} onClick={() => setShowCustom(false)} size="sm" type="button" variant="ghost">Cancel</Button></div>
          <fieldset className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-5" disabled={controlsBusy}>
            <label className="text-xs font-medium text-slate-700">Solution name<input aria-label="Custom solution name" autoFocus className="mt-1.5 h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100" maxLength={80} onChange={(event) => setCustom((current) => ({ ...current, label: event.target.value }))} placeholder="e.g. Client option A" value={custom.label} /></label>
            <CustomNumberField label="PV capacity" onChange={(value) => setCustom((current) => ({ ...current, pv: value }))} suffix="kWp" value={custom.pv} />
            <CustomNumberField label="Battery capacity" min="0" onChange={(value) => setCustom((current) => ({ ...current, battery: value }))} suffix="kWh" value={custom.battery} />
            <CustomNumberField label="PCS capacity" onChange={(value) => setCustom((current) => ({ ...current, inverter: value }))} suffix="kW AC" value={custom.inverter} />
            <CustomNumberField label="Quoted Net CAPEX" onChange={(value) => setCustom((current) => ({ ...current, capex: value }))} prefix="$" suffix="ex GST" value={custom.capex} />
          </fieldset>
          {customValidationError || addCustomError ? <p className="mt-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-sm text-red-800" role="alert">{customValidationError ?? addCustomError}</p> : null}
          <div className="mt-4 flex justify-end"><Button disabled={controlsBusy || customSolutionLimitReached} type="submit">{isAddingCustom ? <RefreshCw aria-hidden="true" className="size-4 animate-spin motion-reduce:animate-none" /> : <Plus aria-hidden="true" className="size-4" />}{isAddingCustom ? "Validating…" : "Add to comparison"}</Button></div>
        </form>
      ) : null}
      <div className="flex flex-wrap items-center justify-between gap-3 border-y border-slate-200 bg-slate-50/70 px-5 py-3 sm:px-6">
        <div className="flex flex-wrap items-center gap-3"><span aria-atomic="true" aria-live="polite" className="text-xs font-semibold tabular-nums text-slate-800">{selectedCount} / {preview.candidate_count} selected</span><span aria-hidden="true" className="h-4 w-px bg-slate-200" /><Button className="h-7 px-2 text-xs text-slate-600" disabled={controlsBusy} onClick={() => onSelectAll(!allSelected)} size="sm" type="button" variant="ghost">{allSelected ? "Clear selection" : "Select all"}</Button></div>
        {isLoading ? <span className="flex items-center gap-2 text-xs text-slate-600" role="status"><RefreshCw aria-hidden="true" className="size-3.5 animate-spin motion-reduce:animate-none" />Updating pricing…</span> : <span className="text-xs text-slate-500">AUD · ex GST</span>}
      </div>
      <div aria-label="Generated solution pricing table" className="ci-scroll-region max-h-[640px] overflow-auto" role="region" tabIndex={0}>
        <table className="w-full min-w-[1040px] table-fixed border-separate border-spacing-0 text-left text-sm">
          ...
          <colgroup><col className="w-[4%]" /><col className="w-[22%]" /><col className="w-[9%]" /><col className="w-[10%]" /><col className="w-[9%]" /><col className="w-[14%]" /><col className="w-[15%]" /><col className="w-[17%]" /></colgroup>
          ...
          <tbody className="divide-y divide-slate-100">{preview.solutions.map((solution, index) => {
            ...
            return <tr className={`group transition-colors hover:bg-slate-50 ${selected ? "bg-cyan-50/25" : "bg-white"}`} key={solution.scenario_id}>
              <td className="border-b border-slate-100 px-2 py-3"><input aria-label={`Select Solution ${index + 1}`} checked={selected} className="size-4 cursor-pointer rounded accent-cyan-700 disabled:cursor-not-allowed" disabled={controlsBusy} onChange={(event) => onSelectionChange(solution.scenario_id, event.target.checked)} type="checkbox" /></td>
              <th className="border-b border-slate-100 py-3 pr-3 font-normal" scope="row"><strong className="font-semibold text-slate-900">Solution {index + 1}</strong><span className="mt-1 block break-words text-xs leading-5 text-slate-500" title={solution.label}>{solution.label}</span></th>
              <td className="whitespace-nowrap border-b border-slate-100 px-2 py-3 tabular-nums">{numberLabel(solution.pv_capacity_kwp_dc)} <span className="text-xs text-slate-500">kWp</span></td>
              ...
              <td className="border-b border-slate-100 px-2 py-3"><div className="relative min-w-0"><span aria-hidden="true" className="pointer-events-none absolute left-2.5 top-2.5 text-slate-400">$</span><input aria-describedby={quoteInvalid ? "analysis-blocker" : undefined} aria-invalid={quoteInvalid || undefined} aria-label={`Quoted Net CAPEX for Solution ${index + 1}`} className={`h-10 w-full min-w-0 rounded-lg border bg-white pl-6 pr-3 text-right text-sm tabular-nums outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100 disabled:cursor-not-allowed disabled:bg-slate-50 ${quoteInvalid ? "border-red-400" : "border-slate-200"}`} disabled={controlsBusy} min="0.01" onChange={(event) => onQuoteChange(solution.scenario_id, event.target.value)} step="0.01" type="number" value={quotes[solution.scenario_id] ?? ""} /></div></td>
            </tr>;
          })}</tbody>
        </table>
      </div>
      <footer className="flex flex-wrap items-center justify-between gap-4 border-t border-slate-200 bg-white p-5 sm:px-6"><div><p className="text-sm font-semibold text-slate-900">{selectedCount} {selectedCount === 1 ? "solution" : "solutions"} selected for analysis</p>{analysisBlocker ? <p className="mt-1 max-w-xl text-xs font-medium text-amber-800" id="analysis-blocker" role="status">{analysisBlocker}</p> : null}</div><Button aria-describedby={analysisBlocker ? "analysis-blocker" : undefined} className="min-w-40 max-sm:w-full" disabled={!validQuotes || controlsBusy} onClick={onAnalyze} type="button">{analysisPending ? <RefreshCw aria-hidden="true" className="size-4 animate-spin motion-reduce:animate-none" /> : <Play aria-hidden="true" className="size-4" />}{analysisPending ? "Analysis running…" : "Analysis"}</Button></footer>
...
function CustomNumberField({ label, min = "0.01", onChange, prefix, suffix, value }: { label: string; min?: string; onChange: (value: string) => void; prefix?: string; suffix: string; value: string }) {
  return <label className="text-xs font-medium text-slate-700">{label}<div className="relative mt-1.5"><input aria-label={label} className={`h-11 w-full rounded-lg border border-slate-300 bg-white ${prefix ? "pl-7" : "pl-3"} pr-16 text-sm tabular-nums outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100`} min={min} onChange={(event) => onChange(event.target.value)} step={prefix ? "0.01" : "any"} type="number" value={value} />{prefix ? <span aria-hidden="true" className="pointer-events-none absolute left-3 top-3 text-slate-400">{prefix}</span> : null}<span className="pointer-events-none absolute right-3 top-3 text-[11px] text-slate-500">{suffix}</span></div></label>;
}
